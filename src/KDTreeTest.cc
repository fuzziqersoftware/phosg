...
void run_randomized_test() {
  phosg::fwrite_fmt(stdout, "-- randomized\n");

  phosg::fwrite_fmt(stdout, "--   construction/insert\n");
  srand(time(nullptr));
  phosg::KDTree<phosg::Vector2<int64_t>, int64_t> t;
  std::set<std::pair<int64_t, int64_t>> points;
  while (points.size() < 1000) {
    int64_t x = rand() % 1000;
    int64_t y = rand() % 1000;
    if (points.emplace(std::make_pair(x, y)).second) {
      t.insert({x, y}, x * y);
    }
  }
  phosg::fwrite_fmt(stderr, "--     tree: size={}, depth={}\n", t.size(), t.depth());

  phosg::fwrite_fmt(stdout, "--   within\n");
  {
    std::set<std::pair<int64_t, int64_t>> remaining_points = points;
    for (const auto& it : t.within({250, 250}, {750, 750})) {
      ...
      expect_eq(1, remaining_points.erase(std::make_pair(it.first.x, it.first.y)));
    }
  ...
  phosg::fwrite_fmt(stdout, "--   erase 1\n");
  {
    // delete all points where x + y is an odd number
    for (auto it = t.begin(); it != t.end();) {
      expect_eq(1, points.count(std::make_pair(it->first.x, it->first.y)));
  ...
  phosg::fwrite_fmt(stdout, "--   erase 2\n");
  {
...
void run_basic_test() {
  phosg::fwrite_fmt(stdout, "-- basic\n");

  phosg::fwrite_fmt(stdout, "--   construction\n");
  ...
  const std::vector<Entry> entries{{2, 3, 0}, {5, 4, 1}, {9, 6, 2}, {4, 7, 3}, {8, 1, 4}, {7, 2, 5}};

  phosg::fwrite_fmt(stdout, "--   insert\n");
  t.insert({2, 3}, 0);
  t.insert({5, 4}, 1);
  t.insert({9, 6}, 2);
  t.insert({4, 7}, 3);
  ...
  phosg::fwrite_fmt(stdout, "--   at/exists\n");
  expect_eq(6, t.size());
  ...
  phosg::fwrite_fmt(stdout, "--   within\n");
  {
    std::set<Entry> remaining_entries;
  ...
  phosg::fwrite_fmt(stdout, "--   iterate\n");
  {
    std::set<Entry> remaining_entries(entries.begin(), entries.end());
  ...
  phosg::fwrite_fmt(stdout, "--   iterate+erase\n");
  {
    std::set<Entry> remaining_entries(entries.begin(), entries.end());
    expect_eq(1, remaining_entries.erase({9, 6, 2}));
    for (auto it = t.begin(); it != t.end();) {
  ...
  phosg::fwrite_fmt(stdout, "--   ensure committed (iterate)\n");
  {
    std::set<Entry> remaining_entries(entries.begin(), entries.end());
    expect_eq(1, remaining_entries.erase({9, 6, 2}));
  ...
  phosg::fwrite_fmt(stdout, "--   ensure committed (within)\n");
  {
    std::set<Entry> remaining_entries;
    remaining_entries.insert({2, 3, 0});
    for (const auto& it : t.within({0, 0}, {5, 10})) {
...
int main(int, char**) {
  run_basic_test();
  run_randomized_test();
  phosg::fwrite_fmt(stdout, "KDTreeTest: all tests passed\n");
  return 0;
}
