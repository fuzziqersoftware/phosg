...
MD5::MD5(const void* data, size_t size) {
  ...
  // Make a copy of the last (possibly incomplete) block, append the trailer, and process what remains. This could
  // result in either one or two blocks
  StringWriter w;
...
std::string MD5::hex() const {
  return std::format(
      "{:08X}{:08X}{:08X}{:08X}", bswap32(this->a0), bswap32(this->b0), bswap32(this->c0), bswap32(this->d0));
}
...
std::string SHA1::hex() const {
  return std::format("{:08X}{:08X}{:08X}{:08X}{:08X}", this->h[0], this->h[1], this->h[2], this->h[3], this->h[4]);
}
