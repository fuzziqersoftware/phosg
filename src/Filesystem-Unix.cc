...
std::string get_user_home_directory() {
  ...
  if (bufsize == -1) {
    throw std::runtime_error("can\'t get _SC_GETPW_R_SIZE_MAX");
  }

  std::string buffer(bufsize, '\0');
  struct passwd pwd, *result = nullptr;
  if (getpwuid_r(getuid(), &pwd, buffer.data(), bufsize, &result) != 0 || !result) {
    throw std::runtime_error("can\'t get home directory for current user");
  }
...
std::unique_ptr<FILE, void (*)(FILE*)> fmemopen_unique(const void* buf, size_t size) {
  return std::unique_ptr<FILE, void (*)(FILE*)>(fmemopen(const_cast<void*>(buf), size, "rb"), fclose_raw);
}

std::shared_ptr<FILE> fmemopen_shared(const void* buf, size_t size) {
  return std::shared_ptr<FILE>(fmemopen(const_cast<void*>(buf), size, "rb"), fclose_raw);
}
...
void make_fd_nonblocking(int fd) {
  ...
  if (flags < 0) {
    throw std::runtime_error("can\'t get socket flags: " + string_for_error(errno));
  }
  if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
    throw std::runtime_error("can\'t set socket flags: " + string_for_error(errno));
  }
}
...
std::pair<int, int> pipe() {
  int fds[2];
  if (::pipe(fds)) {
    throw std::runtime_error("pipe failed: " + string_for_error(errno));
  }
...
std::unordered_map<int, short> Poll::poll(int timeout_ms) {
  if (::poll(this->poll_fds.data(), this->poll_fds.size(), timeout_ms) < 0) {
    if (errno == EINTR) {
      return std::unordered_map<int, short>();
    }
    throw std::runtime_error("poll failed: " + string_for_error(errno));
  }

  std::unordered_map<int, short> ret;
