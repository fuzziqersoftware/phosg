...
std::string name_for_pid(pid_t pid) {
  std::string command = std::format("ps -ax -c -o pid -o command | grep ^\\ *{}\\ | sed s/[0-9]*\\ //g", pid);
...
pid_t pid_for_name(const std::string& name, bool search_commands, bool exclude_self) {
  ...
  for (const auto& it : list_processes(search_commands)) {
    std::string lower_it_name = tolower(it.second);
    ...
    if (pid) {
      throw std::runtime_error("multiple processes found");
    }
    pid = it.first;
  }
  ...
  throw std::out_of_range("no processes found");
}

// calls the specified callback once for each process
std::unordered_map<pid_t, std::string> list_processes(bool with_commands) {
...
bool pid_is_zombie(pid_t pid) {
  ...
  try {
    ...
    if (bytes_read < 0) {
      throw std::runtime_error(std::format("can\'t read stat file for pid {}", pid));
    }
...
uint64_t start_time_for_pid(pid_t pid, bool allow_zombie) {
  ...
  if (ret <= 0) {
    ...
    throw std::runtime_error(std::format("can\'t get start time for pid {}: {}", pid, string_for_error(errno)));
  }
...
Subprocess::Subprocess(const std::vector<std::string>& cmd, int stdin_fd, int stdout_fd,
    int stderr_fd, const std::string* cwd, const std::unordered_map<std::string, std::string>* env)
    : stdin_write_fd(-1), stdout_read_fd(-1), stderr_read_fd(-1), child_pid(0), exit_status(-1) {

  std::set<int> parent_fds_to_close;
  ...
  this->child_pid = fork();
  if (this->child_pid == -1) {
    throw std::runtime_error("fork failed: " + string_for_error(errno));
  }
  if (this->child_pid == 0) { // Child process
...
Subprocess::Subprocess()
    : stdin_write_fd(-1), stdout_read_fd(-1), stderr_read_fd(-1), child_pid(-1), terminated(false), exit_status(-1) {}
...
std::string Subprocess::communicate(const void* stdin_data, size_t stdin_size, uint64_t timeout_usecs) {
  ...
  while ((this->wait(true) < 0) && (now() < deadline_usecs)) {
    ...
    if (events.count(this->stdin_write_fd)) {
      ...
      ssize_t bytes_written = write(
          this->stdin_write_fd, reinterpret_cast<const uint8_t*>(stdin_data) + stdin_offset, bytes_remaining);
  ...
  if (now() >= deadline_usecs) {
    // TODO: we should be a bit more polite here - send SIGTERM, wait a few seconds, then send SIGKILL
    this->kill(SIGKILL);
    throw std::runtime_error("Subprocess::communicate timed out");
  }
  ...
  if (stdout_queue.empty()) {
    ...
    for (const std::string& qi : stdout_queue) {
      ret += qi;
    }
    return ret;
  }
...
std::string Subprocess::communicate(const std::string& stdin_data, uint64_t timeout_usecs) {
...
int Subprocess::wait(bool poll) {
  ...
  for (;;) {
    ...
    if (ret == -1) {
      if (errno != EINTR) {
        throw std::runtime_error("waitpid failed: " + string_for_error(errno));
      }
    } else if (ret == 0) {
...
void Subprocess::kill(int signum) {
  if (::kill(this->child_pid, signum)) {
    throw std::runtime_error("kill failed: " + string_for_error(errno));
  }
}
...
SubprocessResult run_process(const std::vector<std::string>& cmd, const std::string* stdin_data,
  ...
  struct Buffer {
    ...
    size_t offset;
    Buffer(const std::string* buf) : buf(buf), offset(0) {}
  };
  std::unordered_map<int, Buffer> write_fd_to_buffer;
  std::unordered_map<int, std::string*> read_fd_to_buffer;
  ...
  while ((ret.exit_status = sp.wait(true)) == -1) {
    for (const auto& pfd : p.poll(1000)) {
      if (pfd.second & POLLIN) {
        std::string* buf = read_fd_to_buffer.at(pfd.first);
        ...
        if (bytes_read > 0) {
          ...
          throw std::runtime_error("read failed: " + string_for_error(errno));
      ...
      if (pfd.second & POLLOUT) {
        ...
        ssize_t bytes_written = write(pfd.first, buf.buf->data() + buf.offset, bytes_to_write);
        if (bytes_written > 0) {
          ...
          throw std::runtime_error("write failed: " + string_for_error(errno));
  ...
  if (check && sp.wait()) {
    throw std::runtime_error(std::format("command returned code {}\nstdout:\n{}\nstderr:\n{}",
