...
Event::Event(asio::any_io_executor ex) : executor(ex), is_set(false) {}
...
asio::awaitable<void> Event::wait() {
...
SocketReader::SocketReader(asio::ip::tcp::socket&& sock) : sock(std::move(sock)) {}

asio::awaitable<std::string> SocketReader::read_line(const char* delimiter, size_t max_length) {
...
asio::awaitable<std::string> SocketReader::read_data(size_t size) {
  std::string ret;
...
asio::awaitable<void> SocketReader::read_data_into(void* data, size_t size) {
  if (this->pending_data.size() == size) {
...
asio::awaitable<void> sleep(std::chrono::steady_clock::duration duration) {
  asio::steady_timer timer(co_await asio::this_coro::executor, duration);
  co_await timer.async_wait(asio::use_awaitable);
}

asio::awaitable<asio::ip::tcp::socket> connect_tcp(uint32_t ipv4_addr, uint16_t port) {
  uint8_t octets[4] = {
  ...
  return connect_tcp(std::format("{}.{}.{}.{}", octets[0], octets[1], octets[2], octets[3]), port);
}

asio::awaitable<asio::ip::tcp::socket> connect_tcp(const std::string& host, uint16_t port) {
...
asio::awaitable<asio::ip::tcp::socket> connect_tcp(const asio::ip::tcp::endpoint& ep) {
  auto executor = co_await asio::this_coro::executor;
  asio::ip::tcp::socket sock(executor);
