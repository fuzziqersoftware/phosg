...
public:
  ...
  asio::awaitable<T> get() {
    ...
    if (this->exc) {
      ...
      throw std::logic_error("Promise await resolved but did not have a value or exception");
    }
  }
  ...
  void cancel() {
    this->set_exception(std::make_exception_ptr(std::runtime_error("Promise cancelled")));
  }
...
class SocketReader {
public:
  explicit SocketReader(asio::ip::tcp::socket&& sock);
