...
int main(int, char** argv) {
  ...
  root.as_dict().emplace("extra_item", 3);
...

  {
    ...
    string s = root.at("string2");
    expect_eq(s, "no special chars");
    // TODO: It'd be nice to not have to call .as_string here.
    ...
    double f = root.at("float1");
    expect_eq(f, 1.4);
    f = root.at("float0");
    expect_eq(f, 0.0);

    JSON::list_type l = root.at("list1");
    expect_eq(l, JSON::list({1}));
    l = root.at("list0");
    expect_eq(l, JSON::list());

    JSON::dict_type d = root.at("dict1");
    expect_eq(d, JSON::dict({{"one", 1}}));
    d = root.at("dict0");
    expect_eq(d, JSON::dict());
  }
  ...
  expect_eq(root.get("list1", JSON::list_type{2}), JSON::list({1}));
  expect_eq(root.get("missing", JSON::list_type{2}), JSON::list({2}));
  expect_eq(root.get("dict1", JSON::dict_type{{"two", 2}}), JSON::dict({{"one", 1}}));
  expect_eq(root.get("missing", JSON::dict_type{{"two", 2}}), JSON::dict({{"two", 2}}));
  ...
  expect_eq(root.at("list0"), JSON((vector<JSON>())));
  expect_eq(root.at("list1"), JSON((vector<JSON>({{JSON((int64_t)1)}}))));
  expect_eq(root.at("list1").at(0), JSON((int64_t)1));
  expect_eq(root.at("dict0"), JSON(unordered_map<string, JSON>()));
  expect_eq(root.at("dict1"), JSON(unordered_map<string, JSON>({{"one", JSON((int64_t)1)}})));
  ...
  expect_eq(root.at("list0").as_list(), vector<JSON>());
