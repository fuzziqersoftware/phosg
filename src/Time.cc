...
std::string format_time(uint64_t t) {
  ...
  if (len == 0) {
    throw std::runtime_error("format_time buffer too short");
  }
  int usecs_ret = snprintf(ret.data() + len, ret.size() - len, ".%06" PRIu32, static_cast<uint32_t>(t % 1000000));
  if (usecs_ret < 0) {
    throw std::runtime_error("cannot add microsecond field");
  }
  ret.resize(std::min<size_t>(len + usecs_ret, ret.size()));
  return ret;
}
...
std::string format_time_natural(struct timeval* tv) {
...
std::string format_duration(uint64_t usecs, int8_t subsecond_precision) {
  if (usecs < 1000000ULL) {
    ...
    std::string seconds_str = std::format("{:.{}f}", static_cast<double>(usecs_part) / 1000000ULL, subsecond_precision);
