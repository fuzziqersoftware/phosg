...
string get_python_process_output(const string& script, const string* stdin_data) {
  auto r = run_process({"python3", "-c", script}, stdin_data);
...
string get_python3_repr(const string& pickle_data) {
  string script = string_printf("import pickle, sys; print(repr(pickle.load(sys.stdin.buffer)))");
  return get_python_process_output(script, &pickle_data);
}

string get_sorted_dict_python3_repr(const string& pickle_data) {
  string script = string_printf("import pickle, sys; print(repr(sorted(pickle.load(sys.stdin.buffer).items())))");
  return get_python_process_output(script, &pickle_data);
}

string get_python_pickled(const string& repr_data, int protocol) {
  string script = string_printf("import pickle, sys; pickle.dump(eval(sys.stdin.buffer.read()), sys.stdout.buffer, %d)");
...
struct TestCase {
  ...
  string python3_repr;
  string sorted_python3_repr; // for dicts
  JSONObject object;
  ...
  void run() const {
    ...
    if (!this->sorted_python3_repr.empty()) {
      fprintf(stderr, "-- %s (python3 deserialize; sorted)\n", this->name.c_str());
      expected_repr = &this->sorted_python3_repr;
      actual_repr = get_sorted_dict_python3_repr(serialize_pickle(this->object));

    } else if (!this->python3_repr.empty()) {
      fprintf(stderr, "-- %s (python3 deserialize; unsorted)\n", this->name.c_str());
      expected_repr = &this->python3_repr;
      actual_repr = get_python3_repr(serialize_pickle(this->object));
    ...
    for (int protocol = 0; protocol <= 4; protocol++) {
      fprintf(stderr, "-- %s (python3 serialize; protocol %d)\n", this->name.c_str(), protocol);
      auto parsed = try_parse_pickle(get_python_pickled(this->python3_repr, protocol));
      expect_eq(this->object, *parsed);
    }
  }
};
